...
template <int dim>
class EulerFaceHydro : public Hydro<dim> {
protected:
    Mesh::Grid<dim>* grid;
    std::vector<int> insideFaceIds; // IDs of interior faces
public:
    EulerFaceHydro() {}

    EulerFaceHydro(NodeList* nodeList, PhysicalConstants& constants, EquationOfState* eos, Mesh::Grid<dim>* grid) :
        ...
        // Populate list of interior faces
        for (int i = 0; i < grid->size(); ++i) {
            if (!grid->onBoundary(i)) {
                std::vector<int> faceIds = grid->getFaceIds(i);
                insideFaceIds.insert(insideFaceIds.end(), faceIds.begin(), faceIds.end());
            }
        }
    }
    ...
    virtual void EvaluateDerivatives(const State<dim>* initialState, State<dim>& deriv, const double time, const double dt) override {
        ...
        for (int h = 0; h < insideFaceIds.size(); ++h) {
            int faceId = insideFaceIds[h];
            std::vector<int> cellIds = grid->getAdjacentCells(faceId);
            int leftCellId = cellIds[0];
            int rightCellId = cellIds[1];

            double delDotV = 0.0;
            Lin::Vector<dim> delP = Lin::Vector<dim>();

            // Compute velocity gradient using upwind scheme
            for (int d = 0; d < dim; ++d) {
                double dvdx = (velocity->getValue(rightCellId)[d] - velocity->getValue(leftCellId)[d]) / grid->spacing(d);
                delDotV += dvdx;

                // Compute pressure gradient using upwind scheme
                double dPdx = (pressure->getValue(rightCellId) - pressure->getValue(leftCellId)) / grid->spacing(d);
                delP[d] = dPdx;

                // Add artificial viscosity
                double velocityDifference = velocity->getValue(rightCellId)[d] - velocity->getValue(leftCellId)[d];
                delP[d] += artificialViscosityCoefficient * std::abs(velocityDifference) / grid->spacing(d);
            }

            double P = 0.5 * (pressure->getValue(leftCellId) + pressure->getValue(rightCellId));
            double rho = 0.5 * (density->getValue(leftCellId) + density->getValue(rightCellId));
            drhodt->setValue(faceId, -rho * delDotV);
            dvdt->setValue(faceId, -delP / rho);
            dudt->setValue(faceId, -P / rho * delDotV);
        }
    }
