...
template <int dim>
class GridHydroHLLC : public Hydro<dim> {
protected:
    Mesh::Grid<dim>* grid;
...
public:
    ...
    GridHydroHLLC(NodeList* nodeList, PhysicalConstants& constants, EquationOfState* eos, Mesh::Grid<dim>* grid) : 
    ...
    EvaluateDerivatives(const State<dim>* initialState,
        ...
        for (int h = 0; h < insideIds.size(); ++h) {
            ...
            for (int k = 0; k < dim; ++k) {
                ...
                auto flux_L = computeHLLCFlux<dim>(jL, i, k, *rho, *v, *u, *pressure, *soundSpeed);
                auto flux_R = computeHLLCFlux<dim>(i, jR, k, *rho, *v, *u, *pressure, *soundSpeed);
